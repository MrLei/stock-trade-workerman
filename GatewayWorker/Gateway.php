        ...
        switch ($cmd) {
            ...
            case GatewayProtocol::CMD_WORKER_CONNECT:
                ...
                if ($worker_info['secret_key'] !== $this->secretKey) {
                    echo "Gateway: Worker key does not match {$worker_info['secret_key']} !== {$this->secretKey}\n";
                    return $connection->close();
                }
            ...
            case GatewayProtocol::CMD_GATEWAY_CLIENT_CONNECT:
                $worker_info = json_decode($data['body'], true);
                if ($worker_info['secret_key'] !== $this->secretKey) {
                    echo "Gateway: GatewayClient key does not match {$worker_info['secret_key']} !== {$this->secretKey}\n";
                    return $connection->close();
                }
                $connection->authorized = true;
                return;
